...
class TheLivelyThreeSupabaseUser extends BaseAuthUser {
  TheLivelyThreeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> theLivelyThreeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TheLivelyThreeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}

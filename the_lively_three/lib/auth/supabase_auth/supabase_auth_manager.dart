...
class SupabaseAuthManager extends AuthManager with EmailSignInManager {
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      final authUser = user == null ? null : TheLivelyThreeSupabaseUser(user);
  ...
  try {
    ...
    if (existing != null) {
      // ✅ User already exists → return current onboarded status
      return existing['is_onboarded'] ?? false;
    }
    ...
    final res = await SupaFlow.client
        ...
        .insert({
          ...
          'email': email,
          'is_onboarded': false,
          'email_verified': true // default for new users
        })
        .select()
        .maybeSingle();

    print("🆕 New user inserted: $res");

    return res?['is_onboarded'] ?? false;
  } catch (e) {
